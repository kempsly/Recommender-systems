...
class GRU4Rec:
    ...
    ############################LOSS FUNCTIONS######################
    def cross_entropy(self, yhat):
        return tf.reduce_mean(-tf.math.log(tf.linalg.diag_part(yhat)+1e-24))
    def bpr(self, yhat):
        yhatT = tf.transpose(yhat)
        return tf.reduce_mean(-tf.math.log(tf.nn.sigmoid(tf.linalg.diag_part(yhat)-yhatT)))
    def top1(self, yhat):
        yhatT = tf.transpose(yhat)
        term1 = tf.reduce_mean(tf.nn.sigmoid(-tf.linalg.diag_part(yhat)+yhatT)+tf.nn.sigmoid(yhatT**2), axis=0)
        term2 = tf.nn.sigmoid(tf.linalg.diag_part(yhat)**2) / self.batch_size
        return tf.reduce_mean(term1 - term2)

    def build_model(self):
        self.X = tf.keras.layers.Input(shape=(self.batch_size,))
        self.state = [tf.keras.layers.Input(shape=(self.batch_size, self.rnn_size)) for _ in range(self.layers)]

        embedding = tf.keras.layers.Embedding(input_dim=self.n_items, output_dim=self.rnn_size)
        softmax_W = tf.keras.layers.Dense(units=self.rnn_size)
        softmax_b = tf.keras.layers.Dense(units=self.n_items)

        gru_cell = tf.keras.layers.GRU(units=self.rnn_size, activation=self.hidden_act)
        stacked_cell = tf.keras.layers.StackedRNNCells([gru_cell] * self.layers)

        inputs = embedding(self.X)
        output, state = stacked_cell(inputs, tuple(self.state))

        if self.is_training:
            sampled_W = softmax_W(self.Y)
            sampled_b = softmax_b(self.Y)
            logits = tf.matmul(output, tf.transpose(sampled_W)) + sampled_b
            self.yhat = self.final_activation(logits)
            self.cost = self.loss_function(self.yhat)
        else:
        ...
        self.lr = tf.maximum(1e-5, tf.keras.optimizers.schedules.ExponentialDecay(self.learning_rate, self.decay_steps, self.decay, staircase=True))
        optimizer = tf.keras.optimizers.Adam(learning_rate=self.lr)

        tvars = self.trainable_variables
        with tf.GradientTape() as tape:
            gvs = tape.gradient(self.cost, tvars)
            if self.grad_cap > 0:
                gvs = [(tf.clip_by_norm(grad, self.grad_cap), var) for grad, var in zip(gvs, tvars)]
